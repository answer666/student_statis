...
/**
 ...
 @Author：xpl
 @version:  4.3.1 2023-01-13 修复extraParams配置属性名称包含大写，在li元素定义时获取不值问题。如：extraParams=["dateFormat"]， <li dateFormat="yyyy-mm-dd">
 @version:  4.3.0 2022-10-05 优化默认选项配置。例：之前希望将默认“等于”选项改为默认“包含”选项时，必须配置ops，需要配置所有选项，并且还要调整顺序。优化后直接在li标签中配置 firstop="like" 就可以了。
 @version:  4.2.0 2021-07-22 1.新增“多选”操作对标 sql 的 in条件。2.新增复选框编辑器。如：<li field="DynamicCondition.zzmm" title="政治面貌" ops="in" edit="checkbox" templet="党员:dy|团员:ty|群众:qz" ></li>
 @version:  4.1.0 2020-12-31 升级setCondition功能。新增了getCondition方法和setHideCondition方法。并且setCondition支持传入json字符串作为参数
...
layui.extend({
    ...
    "use strict";
    layui.link(layui.cache.base + 'dynamicCondition/dc.css?v='+layui.cache.version);
    ...
    var createInstance = function(_config){
    	var instance = {
    		...
    		,config:_config
    		,requestData:{}
    		,hideCondition:[] //隐藏的查询条件，即buildRequestData时会添加进去提交给服务器，但界面上不显示，用户不知道这些条件（或者知道但这些条件不需要改变）
    	 ...
    	 * */
    	instance.setHideCondition = function(conditionJsonStr){
    		//去除前后空格
    		conditionJsonStr = conditionJsonStr.replace(/(^\s*)|(\s*$)/g, "");
    		conditionJsonStr = conditionJsonStr || "[]";
    		instance.hideCondition = $.parseJSON(conditionJsonStr);
    	}
    	/** 获取最新的查询条件 返回json字符串
    	 * */
    	instance.getCondition = function(){
    		var cacheConditionJsonStr = JSON.stringify(instance.cacheCondition);
    		console.log(cacheConditionJsonStr);
    		return cacheConditionJsonStr;
    	}
    	/**设置初始条件
    	 * conditionArr,老版本为数组，新增支持json字符串。兼容老版本
    	 * */
    	instance.setCondition = function(conditionArr){
    		if(typeof conditionArr == "string"){
    			instance.cacheCondition = $.parseJSON(conditionArr);
    		}else{
...
//			  }
    	  ...
    	  instance.updateConditionValue = function(conditionRowJq, conditionObj){
			  ...
			  if(!conditionObj){
				  if(!conditionOptionVal){
					  if(obj.firstop){
						  conditionOptionVal = obj.firstop;
					  }
					  else if(obj.ops){
...
//				  form.render(null, 'conditionDiv'+instance.conditionContainerId);
    	...
    	instance.buildRequestData = function(cacheCondition){
    		...
    		//concat 2个数组拼接，返回一个新数组，并不改变原来数组
    		var newArr = instance.hideCondition.concat(cacheCondition);
    		//设置请求参数
    		instance.requestData = parseToRequest(newArr ,instance.config.type,instance.config.requestDataType);
...
//    				conditionHtml += fieldText + blankStr + OptionText + blankStr + rsValueText;
    		...
    		if(instance.config.conditionTextId && instance.config.displayModel == "popup"){
    			 ...
    			 msgJq.append($("<div class='xpl-dc-popup-btns'></div>"));
				  ...
				  btnsJq.append($("<a class='layui-btn xpl-popup-query" + hideFlag + "'))>新增查询</a>"));
    	...
    	instance.open = function(){
    		...
    		instance.openPageIndex = layer.open({
    		  ...
    		  // area: [instance.getPopupWidth() + 'px', instance.height +'px'], //宽高
				area: ['60%', '50%'], // 弹窗宽高，手机端适配一般使用百分比
				// shadeClose: true, // 是否点击遮罩关闭弹窗
    		  maxmin:true,
    		...
    		for(var i=0;i<cacheCondition.length;i++){
    			...
    			var conditionFieldVal = conditionRowJq.find("select[name='conditionField']").val();
	  			...
	  			if(item.allowDel === "false"){
					...
					conditionRowJq.find(".delRowBtn").addClass("xpl-conditionRow-x");
	  				//移除其他下拉选项
...
//    				  layer.msg('校验失败');
    ...
    var defaultValueConfig = {
    		...
    		opsMap: {"equal":"等于","like":"包含","between":"范围","in":"多选"
...
//		    	form.render('select')
    ...
    $.extend( editorCheckbox ,{
		createElement:function(item){
	  		var randomId= (Math.random()+"").substr(2);
	  		var ele = $("<div class='xpl-dc-value xpl-dc-value-checkbox' style=''>");
	  		if(item.templet){
	  			if(item.templet.startsWith("#")){
	  				var selJq = $(item.templet);
	  				if(selJq.is("select")){
	  					selJq.find("option").each(function() {
	  		  				var val = $(this).val();
	  		  				var text = $(this).html();
	  		  				if(val){
	  		  					ele.append('<input type="checkbox" name="rd'+randomId+'" value="'+val+'" title="'+text+'"/>');
	  		  				}
	  		  		    });
					}else{
						console.error("多选框定义出错");
					}

	  			}else{
	  				var arr = item.templet.split("|")
		  			for (var i = 0; i < arr.length; i++) {
		  				var text = arr[i];
						var val = arr[i];
						var textVal = arr[i].split(":");
						if(textVal.length == 2){
							text = textVal[0];
							val = textVal[1];
						}
		...
		,fillElement:function(ele, val){
			...
			if(val.value){
				var checkedValArr = val.value.split(",");
				for(var i=0;i<checkedValArr.length;i++){
					$(ele).find('input[value="'+checkedValArr[i]+'"]').prop('checked', true);
				}
			}
		}
  		,getRequestValue:function(ele){
...
//   		 * conditionValueVal,conditionValueLeftVal,conditionValueRightVal
	...
	function fillExtraParams(obj,extraParams,item){
		...
		for(var i=0;i<extraParams.length;i++){
			...
			obj[paramName] = getValByKeyIgnoreCase(item, paramName);
		}
	}
	/***
	 * 根据对象key获取对应的值。与obj[key]区别是，该方法忽略key的大小写。所以如果obj属性不考虑大小写有属性则结果异常。
	 * @param obj
	 * @param key
	 * @returns
	 */
	function getValByKeyIgnoreCase(obj, key){
		for(var keyStr in obj){
	        if(key.toLocaleLowerCase() == keyStr.toLocaleLowerCase() ){
	        	return obj[keyStr];
	        }
	    }
		return null;
